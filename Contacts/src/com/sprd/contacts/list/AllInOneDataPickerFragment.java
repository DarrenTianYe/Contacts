...
public class AllInOneDataPickerFragment extends ContactEntryListFragment<ContactEntryListAdapter>
	...
	protected void prepareEmptyView() {
        ...
        if (mCascadingData.size() == 1 && mCascadingData.contains(Email.CONTENT_ITEM_TYPE)) {
    ...
    public void onMultiPickerSelected() {
        ...
        for (Integer i : checkedItems) {
            ...
            int columnIndex = c.getColumnIndex(Data.DATA1);
            if (columnIndex == -1) {
                continue;
            }

            String num = c.getString(c.getColumnIndex(Data.DATA1));
            String name = c.getString(c.getColumnIndex(Phone.DISPLAY_NAME_PRIMARY));
            if (c.getInt(c.getColumnIndex(Contacts.DISPLAY_NAME_SOURCE)) != 40) {
                name = null;
            }
            ret.put(num, name);
        }
