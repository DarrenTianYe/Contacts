 ...
 */
	...
	String tmp=getUserData(new Account(account.name,account.type),mimeType+"_typeoverallmax");
	int ret=0;
	if (tmp!=null) {
	...
	int currentLength=0;
	int maxLength=-1;
	String simCardLengthStr=AccountManager.get(mContext).getUserDataPrivileged(new Account(account.name,account.type),"capacity");
	...
	for (RawContactDelta state : set) {
		...
		String tmp=am.getUserDataPrivileged(account,mimeType+"_length");
		int max=-1;
		if (tmp!=null) {
	...
	for (RawContactDelta state : set) {
		...
		String tmp=am.getUserDataPrivileged(account,mimeType+"_capacity");
		int max=-1;
		if (tmp!=null) {
		    max=Integer.parseInt(tmp);
		} 
	...
	try {
	    bytes = GsmAlphabet.stringToGsmAlphaSS(record);
    ...
    public String getUserData(Account account, String key) {
	    ...
	    if (ret == null) {
	        ret = mAm.getUserDataPrivileged(account, key);
	        if (ret == null) {
		        ret = "nil";
	        } 
	        datas.put(key, ret);
