...
public class ContactsIntentResolver {
    ...
    public ContactsRequest resolveIntent(Intent intent) {
        ...
        Log.d(TAG, "Called with action: " + action+"intent.getsomthing"+intent.getExtra("fast_dial"));
         ...
         * @{
         */
        else if (UI.MULTI_PICK_ACTION.equals(action)) {
            final String resolvedType = intent.resolveType(mContext);
            if (Contacts.CONTENT_TYPE.equals(resolvedType)) {
                request.setActionCode(ContactsRequest.ACTION_MULTI_PICK_CONTACT);
            } else if (Phone.CONTENT_TYPE.equals(resolvedType)) {
                request.setActionCode(ContactsRequest.ACTION_MULTI_PICK_PHONE);
            } else if (Email.CONTENT_TYPE.equals(resolvedType)) {
                request.setActionCode(ContactsRequest.ACTION_MULTI_PICK_EMAIL);
			} else {
				/**
				 * add by xuhong.tian
				 */
				Log.d(TAG, "coming..............ss=="+intent.getBooleanExtra("private_contacts", false));
				if (intent.getExtra("private_contacts") != null
						&& (intent.getBooleanExtra("private_contacts", false))) {
					
					Log.d(TAG, "coming..............=="+intent.getBooleanExtra("private_contacts", false));
					List<String> cascadingDate = resolveCascadingDate(intent);
					if (!cascadingDate.isEmpty()) {
			    		intent.putExtra("private_contacts", 1);
			    		request.setSmsFlag(1);
						request.setActionCode(ContactsRequest.ACTION_MULTI_PICK_ALL_IN_ONE_DATA);
						request.setCascadingData(cascadingDate);
					}
				} else {
					List<String> cascadingDate = resolveCascadingDate(intent);
					if (!cascadingDate.isEmpty()) {
        ...
        } else if ("com.android.contacts.action.MULTI_TAB_PICK".equals(action)) {
            List<String> cascadingDate = resolveCascadingDate(intent);
            if (!cascadingDate.isEmpty()) {
                request.setActionCode(ContactsRequest.ACTION_MULTI_PICK_ALL_IN_ONE_DATA_NEWUI);
                request.setCascadingData(cascadingDate);
            }
        }
        /**
        ...
        // so we need to re-dispatch from here to the intended target.
            ...
            intent.setAction(Intent.ACTION_DEFAULT);
            intent.setData(null);
        }else if("android.intent.action.PRIVATE_GET_CONTENT".equals(action)){
        	
            String type = intent.getType();
            if (Contacts.CONTENT_ITEM_TYPE.equals(type)) {
                request.setActionCode(ContactsRequest.ACTION_PICK_OR_CREATE_CONTACT);
            } else if (Phone.CONTENT_ITEM_TYPE.equals(type)) {
                request.setActionCode(ContactsRequest.ACTION_VIEW_CONTACT_SELECT_FAST_CALL);
            } else if (Phones.CONTENT_ITEM_TYPE.equals(type)) {
                request.setActionCode(ContactsRequest.ACTION_PICK_PHONE);
                request.setLegacyCompatibilityMode(true);
            } else if (StructuredPostal.CONTENT_ITEM_TYPE.equals(type)) {
     ...
     * @{
     */
    List<String> resolveCascadingDate(Intent intent) {
        if (intent == null) {
        ...
        while (true) {
            currentIntent = (Intent) currentIntent.getParcelableExtra("cascading");
            if (currentIntent == null) {
                break;
            }
            String mimeType = currentIntent.getType();
            if (mimeType != null) {
                ret.add(mimeType);
            }
        }
        if (Constants.DEBUG)
            Log.d(TAG, "resolveCascadingDate:" + ret);
        return ret;
    }
    /**
     * @}
