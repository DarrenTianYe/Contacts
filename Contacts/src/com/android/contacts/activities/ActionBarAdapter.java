...
import com.sprd.contacts.util.UniverseThemeUtils;
...
public class ActionBarAdapter implements OnQueryTextListener, OnCloseListener {
    ...
    public ActionBarAdapter(Context context, Listener listener, ActionBar actionBar,
        ...
        /**
        * SPRD:
        *  Modify this sentence for UniverseUI.
        * Original Android code:
        * if (isUsingTwoPanes)
        * @{
        */
        if (isUsingTwoPanes && !UniverseUtils.UNIVERSEUI_SUPPORT) {
            /**
            * @}
            */
    ...
    private void setupTabs() {
        ...
        */
        if (UniverseUtils.UNIVERSEUI_SUPPORT) {
            addTabNewUI(TabStateNewUI.ALL, R.drawable.ic_tab_all_newui, R.string.people);
            addTabNewUI(TabStateNewUI.GROUPS, R.drawable.ic_tab_groups_newui,
                    R.string.contactsGroupsLabel);
            addTabNewUI(TabStateNewUI.FAVORITES, R.drawable.ic_tab_starred_newui,
                    R.string.contactsFavoritesLabel);
            addTabNewUI(TabStateNewUI.MORE, R.drawable.ic_tab_more_newui, R.string.tab_more);
		} else {
...
//        addTab(TabState.FAVORITES, R.drawable.ic_tab_starred, R.string.contactsFavoritesLabel);
    ...
    public void initialize(Bundle savedState, ContactsRequest request) {
        if (savedState == null) {
            ...
            */
            if(UniverseUtils.UNIVERSEUI_SUPPORT){
                mCurrentTabNewUI = loadLastTabPreferenceNewUI();
            } else {
                mCurrentTab = loadLastTabPreference();
            } 
//            mCurrentTab = loadLastTabPreference();
            ...
            */
            if(UniverseUtils.UNIVERSEUI_SUPPORT){
                mCurrentTabNewUI = savedState.getInt(EXTRA_KEY_SELECTED_TAB_NEWUI);
            } else {
                mCurrentTab = savedState.getInt(EXTRA_KEY_SELECTED_TAB);
            } 
//            mCurrentTab = savedState.getInt(EXTRA_KEY_SELECTED_TAB);
    ...
    private class MyTabListener implements ActionBar.TabListener {
        ...
        @Override public void onTabSelected(Tab tab, FragmentTransaction ft) {
            if (!mIgnoreTabSelected) {
                ...
                */
                if(UniverseUtils.UNIVERSEUI_SUPPORT){
                    setCurrentTabNewUI(tab.getPosition());
                } else {
                    setCurrentTab(tab.getPosition());
                }
//                setCurrentTab(tab.getPosition());
        ...
        if (!UniverseUtils.UNIVERSEUI_SUPPORT) {
            ...
            if (isIconifiedChanging) {
                if (mSearchMode) {
                    ...
                    // Since we have the {@link SearchView} in a custom action
                    // bar, we must manually handle
                    // collapsing the {@link SearchView} when search mode is
                    // exited.
...
//            }
          //SPRD:Bug258438
            ...
            if (mActionBarNavigationMode == ActionBar.NAVIGATION_MODE_TABS
                ...
                */
                if(UniverseUtils.UNIVERSEUI_SUPPORT){
                    mActionBar.setSelectedNavigationItem(mCurrentTabNewUI);
                } else {
                    mActionBar.setSelectedNavigationItem(mCurrentTab);
                }
//                mActionBar.setSelectedNavigationItem(mCurrentTab);
...
//            }
    ...
    * @{
    */ 
    public interface TabStateNewUI  {
    ...
    private static final int DEFAULT_TAB_NEWUI = TabStateNewUI.DEFAULT;
    private int mCurrentTabNewUI = TabStateNewUI.ALL;
    private static final String PERSISTENT_LAST_TAB_NEWUI = "actionBarAdapter.lastTabNewUI";
    private static final String EXTRA_KEY_SELECTED_TAB_NEWUI = "navBar.selectedTabNewUI";
    
    public int getCurrentTabNewUI() {
        return mCurrentTabNewUI;
    }

    private void addTabNewUI(int tabStateNewUI, int icon, int description) {
        final Tab tab = mActionBar.newTab();
        tab.setTag(tabStateNewUI);
        tab.setTabListener(mTabListener);
        if (UniverseUtils.UNIVERSEUI_SUPPORT
                && UniverseThemeUtils.isUsingAlternativeTabStyle(mContext)) {
            tab.setIcon(icon);
            tab.setText(description);
        } else {
            if (mShowTabsAsText) {
    ...
    public void setCurrentTabNewUI(int tab) {
        setCurrentTabNewUI(tab, true);
    }

    public void setCurrentTabNewUI(int tab, boolean notifyListener) {
        if (tab == mCurrentTabNewUI) {
            return;
        }
        mCurrentTabNewUI = tab;

        final int actionBarSelectedNavIndex = mActionBar.getSelectedNavigationIndex();
        switch (mActionBar.getNavigationMode()) {
            case ActionBar.NAVIGATION_MODE_TABS:
                if (mCurrentTabNewUI != actionBarSelectedNavIndex) {
                    mActionBar.setSelectedNavigationItem(mCurrentTabNewUI);
                }
                break;
            case ActionBar.NAVIGATION_MODE_LIST:
                if (mCurrentTabNewUI != getTabPositionFromNavigationItemPosition(actionBarSelectedNavIndex)) {
                    mActionBar.setSelectedNavigationItem(
                            getNavigationItemPositionFromTabPosition(mCurrentTabNewUI));
                }
                break;
        }
        ...
        if (notifyListener && mListener != null)
            mListener.onSelectedTabChanged();
        saveLastTabPreferenceNewUI(mCurrentTabNewUI);
    }

    private void saveLastTabPreferenceNewUI(int tab) {
        mPrefs.edit().putInt(PERSISTENT_LAST_TAB_NEWUI, tab).apply();
    }

    private int loadLastTabPreferenceNewUI() {
        try {
            return mPrefs.getInt(PERSISTENT_LAST_TAB_NEWUI, DEFAULT_TAB_NEWUI);
        } catch (IllegalArgumentException e) {
            // Preference is corrupt?
            return DEFAULT_TAB_NEWUI;
        }
    }
    /**
    * @}
