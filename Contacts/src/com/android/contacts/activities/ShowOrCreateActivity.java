...
public final class ShowOrCreateActivity extends ContactsActivity
    ...
    protected void onCreate(Bundle icicle) {
        ...
        */
         ...
         * add by xuhong.tian
         */
        private_call_flag=intent.getIntExtra("private_call", 4);
        mNumber = (String) intent.getExtra("private_number", "0");
        Log.d(TAG, "private_call_flag=="+private_call_flag+"mNumber=="+mNumber);
    }
...
//                showDialog(CREATE_CONTACT_DIALOG);
    ...
    protected Dialog onCreateDialog(int id) {
	    ...
	    case CREATE_CONTACT_DIALOG:
			...
			if (private_call_flag != Calls.PUBLIC_CALL_FLAG) {
				...
				AccountWithDataSet account = new AccountWithDataSet(
						...
						PhoneAccountType.PRIVATE_ACCOUNT_TYPE, null);
				createIntent.putExtra(Intents.Insert.ACCOUNT, account);
				createIntent.putExtra(Insert.PHONE, mNumber);
				Bundle extras = new Bundle();
				extras.putInt("public_create", 1);
				createIntent.putExtras(extras);
			} else {
				createIntent = new Intent(Intent.ACTION_INSERT_OR_EDIT);
				createIntent.putExtras(mCreateExtras);
