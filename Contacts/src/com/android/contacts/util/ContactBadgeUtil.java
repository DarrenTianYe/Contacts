...
public class ContactBadgeUtil {
    ...
    /**
     * SPRD:
     * Modify this method when merge from 4.1 to 4.3
     * 
     * Original Android code:
     * The code be remarked.
     * @{
     */
    public static synchronized Bitmap loadDefaultAvatarPhoto(Context context, boolean hires,
            boolean darkTheme) {
        int resId = ContactPhotoManager.getDefaultAvatarResId(hires, darkTheme);
        Bitmap ret = sDefaultPhotos.get(resId);
        if (ret == null) {
            ret = BitmapFactory.decodeResource(context.getResources(), resId);
            sDefaultPhotos.put(resId, ret);
        }
        return ret;
    }
...
//    }
    ...
    * @{
    */
    private static Map<Integer,Bitmap> sDefaultPhotos=new HashMap<Integer,Bitmap>();
    /**
    * @}
    */
}
