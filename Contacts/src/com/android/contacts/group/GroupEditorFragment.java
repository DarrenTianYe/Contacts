...
import com.android.contacts.common.GroupMemberLoader.GroupEditorQuery;
...
public class GroupEditorFragment extends Fragment implements SelectAccountDialogFragment.Listener {
    ...
    /**
    * SPRD:
    *   The following code is added by sprd.We use the button to select the contact
    *   in sprd platform.
    *
...
//        }
    ...
    private void setupEditorForAccount() {
        ...
        if (UniverseUtils.UNIVERSEUI_SUPPORT) {
            newGroupEditorId =
                    editable ? R.layout.group_editor_view_overlay
...
//            mAutoCompleteAdapter = null;
        ...
        * @{
        */
        mGroupNameView.setInputType(InputType.TYPE_TEXT_FLAG_NO_SUGGESTIONS);
        /*
        * @}
        */
        /**
        ...
        if (UniverseUtils.UNIVERSEUI_SUPPORT) {
            ...
            if (isPortrait == false) {
                ViewTreeObserver viewObserver = mSelectGroupMember.getViewTreeObserver();
                viewObserver.addOnPreDrawListener(new ViewTreeObserver.OnPreDrawListener() {
                    public boolean onPreDraw() {
                        int btnViewWidth = mSelectGroupMember.getMeasuredWidth();
                        LinearLayout.LayoutParams layoutParams = new LinearLayout.LayoutParams(
                                LayoutParams.WRAP_CONTENT,
                                LayoutParams.WRAP_CONTENT);
                        layoutParams.width = btnViewWidth + 6;
            ...
            if (!TextUtils.isEmpty(mAccountName)) {
                 ...
                 */
                 if (PhoneAccountType.ACCOUNT_TYPE.equals(mAccountType)) {
                     accountNameTextView.setText(
                            mContext.getString(R.string.from_account_format, mContext.getString(R.string.label_phone)));
                 }else {
                     accountNameTextView.setText(
                            mContext.getString(R.string.from_account_format, mAccountName));
                 }
...
//    }
    ...
    */
    ...
    public boolean save(int savdMode, boolean backPressed) {
         ...
         */
        if (Log.isIloggable()) {
            Log.startPerfTracking(Constants.PERFORMANCE_TAG + String.format(": Start adding %d contacts to group", mListMembersToAdd.size()));
        }
         /**
         * @}
         */
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if (convertView == null) {
                ...
                */
                if (UniverseUtils.UNIVERSEUI_SUPPORT) {
                    result = mLayoutInflater.inflate(mIsGroupMembershipEditable ?
                            R.layout.group_member_item_overlay
    ...
    */
    ...
    private int limitTextEditorLength(String txtString) {
        ...
        if (TextFieldsEditorView.isChinese(txtString)) {
            maxLen = maxLen - 1;
        }
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == SUBACTIVITY_SELECT_GROUP_MEMBER) {
            if (resultCode == Activity.RESULT_OK) {
                ...
                Toast.makeText(getActivity(), R.string.contact_list_loading, Toast.LENGTH_LONG)
                        .show();
            } else {
				mIsLoading = false;
				mSelectGroupMember.setEnabled(true);
