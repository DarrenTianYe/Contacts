...
public class AccountFilterUtil {
    ...
    /**
     * Find TextView with the id "account_filter_header" and set correct text
     * for the account filter header.
     * 
     * @param filterContainer View containing TextView with id
     *            "account_filter_header"
     * @return true when header text is set in the call. You may use this for
     *         conditionally showing or hiding this entire view.
     */
    ...
    /**
     * Similar to
     * {@link #updateAccountFilterTitleForPeople(View, ContactListFilter, boolean, boolean)}
     * , but for Phone UI.
     */
     ...
     * @{
     */
        public static boolean updateAccountFilterTitle(View filterContainer,
                ContactListFilter filter, boolean showTitleForAllAccounts,
                int resId) {
        /**
         * @}
        ...
        */
        ...
        if (filter != null) {
            if (filter.filterType == ContactListFilter.FILTER_TYPE_ALL_ACCOUNTS) {
                if (showTitleForAllAccounts) {
                    headerTextView.setText(resId);
                    textWasSet = true;
                }
            } else if (filter.filterType == ContactListFilter.FILTER_TYPE_ACCOUNTS) {
                headerTextView.setText(R.string.contactPickerActivityTitle);
                textWasSet = true;
            } else if (filter.filterType == ContactListFilter.FILTER_TYPE_ACCOUNT) {
                int capacity = getAccountCapacity(filterContainer.getContext(), filter);
                if (PhoneAccountType.ACCOUNT_TYPE.equals(filter.accountType)) {
                    text = context.getString(R.string.listAllContactsInAccount,
                            context.getString(R.string.public_contacts));
                } else {
                    text = context.getString(R.string.listAllContactsInAccount, filter.accountName);
                }
/*                if (capacity != -1) {
                    text += " (" + context.getString(R.string.listMaxContactsInAccount, capacity)
                            + ")";
                }*/
                 ...
                 * add by xuhong.tian
                 */
            } else if (filter.filterType == ContactListFilter.FILTER_TYPE_PRIVATECONTACTS) {
                headerTextView.setText(R.string.show_privatecontacts);
                textWasSet = true;
            } else {
                Log.w(TAG, "Filter type \"" + filter.filterType + "\" isn't expected.");
                headerTextView.setText(R.string.list_filter_phones);
    ...
    * @param filter 
    * @return
    * -1    infinity or account doesn't support it
    * >=0   account capacity
    * @{
    */
...
/*        if (filter.filterType != ContactListFilter.FILTER_TYPE_ACCOUNT) {
           // return -1;
            ...
            String tmp = AccountManager.get(context).getUserDataPrivileged(account, "capacity");
            if (tmp == null) {
                return -1;
            }
