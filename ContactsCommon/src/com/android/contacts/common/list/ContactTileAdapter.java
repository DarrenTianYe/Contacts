...
import com.android.contacts.common.GroupMemberLoader.GroupDetailQuery;
...
public class ContactTileAdapter extends BaseAdapter {
    ...
    public ContactTileAdapter(Context context, ContactTileView.Listener listener, int numCols,
        ...
        */
        if (UniverseUtils.UNIVERSEUI_SUPPORT){
            // mPaddingInPixels = mContext.getResources()
            // .getDimensionPixelSize(R.dimen.contact_tile_divider_padding_new);
            //NEWUI 8dip
            mPaddingInPixels = 8;
        }else{
            mPaddingInPixels = mContext.getResources()
    ...
    protected int getDividerPosition(Cursor cursor) {
        if (cursor == null || cursor.isClosed()) {
            ...
            * @{
            */
            Log.e(TAG, "Unable to access cursor");
            if (!((Activity)mContext).isFinishing()) {
		Log.e(TAG, "finish the activity");
    ...
    protected ContactEntry createContactEntryFromCursor(Cursor cursor, int position) {
        ...
        * @{
        */
        contact.contactId = cursor.getInt(mIdIndex);
        /**
        * @}
        */
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        */
        if (UniverseUtils.UNIVERSEUI_SUPPORT){
            contactTileRowView.configureRowNew(contactList, position == getCount() - 1 , position == 0);
        }
        /**
        * @}
        */
    ...
    private int getLayoutResourceId(int viewType) {
        switch (viewType) {
            case ViewTypes.STARRED:
                ...
                */
                if (UniverseUtils.UNIVERSEUI_SUPPORT) {
                    return mIsQuickContactEnabled ? R.layout.contact_tile_starred_quick_contact
                            : R.layout.contact_tile_starred_overlay;
                }
                /**
                * @}
                */
            ...
            case ViewTypes.FREQUENT:
                ...
                */
                if (UniverseUtils.UNIVERSEUI_SUPPORT) {
                    return mDisplayType == DisplayType.STREQUENT_PHONE_ONLY ? R.layout.contact_tile_frequent_phone_overlay
                            : R.layout.contact_tile_frequent_overlay;
                }
                /**
                * @}
                */
            ...
            case ViewTypes.STARRED_PHONE:
                ...
                */
                if (UniverseUtils.UNIVERSEUI_SUPPORT) {
                    return R.layout.contact_tile_starred_secondary_target_overlay;
                }
                /**
                * @}
                */
    ...
    private class ContactTileRow extends FrameLayout {
        ...
        private void onMeasureForTiles(int widthMeasureSpec) {
            ...
            * @{
            */
            final int totalPaddingsInPixels;
            if (UniverseUtils.UNIVERSEUI_SUPPORT){
                totalPaddingsInPixels = mColumnCount * mPaddingInPixels;
            }else{
                totalPaddingsInPixels = (mColumnCount - 1) * mPaddingInPixels;
            }
            /**
            * @}
            */
        ...
        * @{
        */
        public void configureRowNew(ArrayList<ContactEntry> list, boolean isLastRow , boolean isFirstRow) {
            int columnCount = mItemViewType == ViewTypes.FREQUENT ? 1 : mColumnCount;
            ...
            for (int columnCounter = 0; columnCounter < columnCount; columnCounter++) {
                ContactEntry entry =
                        columnCounter < list.size() ? list.get(columnCounter) : null;
                        addTileFromEntryNew(entry, columnCounter, isLastRow , isFirstRow);
            }
        }

        private void addTileFromEntryNew(ContactEntry entry, int childIndex, boolean isLastRow,
                boolean isFirstRow) {
            final ContactTileView contactTile;

            if (getChildCount() <= childIndex) {
                contactTile = (ContactTileView) inflate(mContext, mLayoutResId, null);
                // Note: the layoutparam set here is only actually used for
                // FREQUENT.
                // We override onMeasure() for STARRED and we don't care the layout
                // param there.
                Resources resources = mContext.getResources();
                contactTile.setPhotoManager(mPhotoManager);
                contactTile.setListener(mListener);
            ...
            switch (mItemViewType) {
                ...
                case ViewTypes.STARRED:
                    // Setting divider visibilities
                    contactTile.setPadding(mPaddingInPixels, mPaddingInPixels, mPaddingInPixels,
                            mPaddingInPixels);
                    break;
                case ViewTypes.FREQUENT:
                    contactTile.setHorizontalDividerVisibility(
                            View.VISIBLE);
                    if (isLastRow)
                        contactTile.setPadding(0, 0, 0, 5);
                    break;
                default:
                    break;
            }
