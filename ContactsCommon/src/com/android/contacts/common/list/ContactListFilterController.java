...
public abstract class ContactListFilterController {
    ...
    public static ContactListFilterController getInstance(Context context) {
        ...
        if (sFilterController == null) {
             ...
             * @{
             */
            synchronized (ContactListFilterController.class) {
                if (sFilterController == null) {
                    sFilterController = new ContactListFilterControllerImpl(context);
                }
            }
...
class ContactListFilterControllerImpl extends ContactListFilterController {
    ...
    public ContactListFilterControllerImpl(Context context) {
        mContext = context;
        mFilter = ContactListFilter.restoreDefaultPreferences(getSharedPreferences(),mContext);
    ...
    private void setContactListFilter(ContactListFilter filter, boolean persistent,
        ...
        * @{
        */
        if (mFilter != null && mFilter.equals(filter) && filter.onlyPhonesChanged == 0
                && filter.filterType != ContactListFilter.FILTER_TYPE_CUSTOM) {
            return;
        }
        filter.onlyPhonesChanged = 0;
        /*
        * @}
        */
        mFilter = filter;
        ...
        switch (mFilter.filterType) {
            case ContactListFilter.FILTER_TYPE_SINGLE_CONTACT:
                setContactListFilter(
                        ContactListFilter.restoreDefaultPreferences(getSharedPreferences(),mContext),
                        false, notifyListeners);
                break;
            case ContactListFilter.FILTER_TYPE_ALL_ACCOUNTS:
                setContactListFilter(ContactListFilter.createFilterWithType(
                        ContactListFilter.FILTER_TYPE_ALL_ACCOUNTS, mContext), true,
                        notifyListeners);
                break;
            case ContactListFilter.FILTER_TYPE_ACCOUNT:
                ...
                * @{
                */
                if (!filterAccountExists() || isOnlyPhoneAccount()) {
                    // The current account filter points to invalid account. Use
                    // "all" filter
                    // instead.
                    setContactListFilter(ContactListFilter.createFilterWithType(
                            ContactListFilter.FILTER_TYPE_ALL_ACCOUNTS, mContext), true,
    ...
    */
    /**
     * @return true if only phone Account exists.
     */
    private boolean isOnlyPhoneAccount() {
        final AccountTypeManager accountTypeManager = AccountTypeManager.getInstance(mContext);
        final ArrayList<AccountWithDataSet> allAccounts = (ArrayList) accountTypeManager
                .getAccounts(false);
