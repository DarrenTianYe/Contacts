...
public class PhoneNumberListAdapter extends ContactEntryListAdapter {
    ...
    public static class PhoneQuery {
        public static final String[] PROJECTION_PRIMARY = new String[] {
            ...
            */
             ...
             * add by xuhong.tian
             */
    	    Contacts.CONTACT_IMPORT_TYPE,
    	    Contacts.CONTACAT_MODE_TYPE,
};
        ...
        */
         ...
         * add by xuhong.tian
         */
        public static final int CONTACT_DISPLAY_IMPORT_TYPE = 11;
        public static final int CONTACT_DISPLAY_MODE_TYPE = 12;
    }
    ...
    public void configureLoader(CursorLoader loader, long directoryId) {
    	...
    	Log.d(TAG, "configureLoader>>>>"+isExtendedDirectory(directoryId));
        ...
        if (isExtendedDirectory(directoryId)) {
            ...
            if (isSearchMode()) {
				...
				if (getFilter().filterType == ContactListFilter.FILTER_TYPE_PRIVATECONTACTS) {

					loader.setSelection(Contacts.CONTACAT_MODE_TYPE + " = "
							+ String.valueOf(mMode));

				} else if (getFilter().filterType == ContactListFilter.FILTER_TYPE_ACCOUNT) { // local
																								// account

					loader.setSelection(Contacts.CONTACT_IMPORT_TYPE + " < "
							+ String.valueOf(RawContacts.CONTACTS_BY_PCTOOLS));
				} else {
					loader.setSelection(Contacts.CONTACT_IMPORT_TYPE + " = "
							+ String.valueOf(RawContacts.CONTACTS_BY_LOCAL)
							+ " OR " + Contacts.CONTACAT_MODE_TYPE + " = "
							+ String.valueOf(mMode));
				}
            } else {
            	
            	
            	Log.d(TAG, "configureLssoader>>"+isSectionHeaderDisplayEnabled()+"getFilter()=="+getFilter());
...
//			selection.append(Contacts.CONTACT_IMPORT_TYPE + " = "
//					+ String.valueOf(RawContacts.CONTACTS_BY_LOCAL));
//			selection.append(" OR " + Contacts.CONTACAT_MODE_TYPE + " = "
//					+ String.valueOf(mMode));
//
...
/*    			selection.append(Contacts.CONTACT_IMPORT_TYPE + " = "
    					+ String.valueOf(RawContacts.CONTACTS_BY_LOCAL));
    			*/
            	Log.d(TAG, "xxxxxxxxxxxxxxxxxxxxxxxxxxx>>>>>>>>>>>>>>>>>"+mMode);
    ...
    public String getPhoneMode(int position) {
        final Cursor item = (Cursor)getItem(position);
        return item != null ? item.getString(PhoneQuery.CONTACT_DISPLAY_IMPORT_TYPE) : null;
    }
                ...
                */
				...
				if (getDisplayPhotos()) {
					...
					bindQuickContact(view, partition, cursor,
							PhoneQuery.PHOTO_ID, PhoneQuery.PHOTO_URI,
							PhoneQuery.CONTACT_ID, PhoneQuery.LOOKUP_KEY,
							PhoneQuery.CONTACT_DISPLAY_ACCOUNT_TYPE,
							PhoneQuery.CONTACT_DISPLAY_ACCOUNT_NAME);
				}
    ...
    protected void bindPhoneNumber(ContactListItemView view, Cursor cursor, boolean displayNumber) {
		...
		if (displayNumber && !cursor.isNull(PhoneQuery.PHONE_TYPE)) {
			...
			import_type = cursor.getInt(PhoneQuery.CONTACT_DISPLAY_IMPORT_TYPE);
		...
		/**
		 * add by xuhong.tian on 11.3 that public people attribute show public
		 * and private people attribute show private
		 */
		Log.d(TAG, "import_type===" + import_type);
		if (RawContacts.CONTACTS_BY_PCTOOLS == import_type
				|| RawContacts.CONTACTS_BY_EDITE == import_type) {
			view.setLabel(mContext.getString(R.string.private_number));
		} else {
			view.setLabel(label);
		}
    ...
    */
    ...
    public String getQuantityText(int count, int zeroResourceId, int pluralResourceId) {
        if (count == 0) {
            ...
            return getContext().getString(R.string.local_search_label);
        }
    }
    /**
    * @}
