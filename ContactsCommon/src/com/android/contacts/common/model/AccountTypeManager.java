...
public abstract class AccountTypeManager {
    ...
    */
    ...
    protected int[] simIconRes = {
            R.drawable.icon_sim1_sprd, R.drawable.icon_sim2_sprd,
            R.drawable.icon_sim3_sprd, R.drawable.icon_sim4_sprd, R.drawable.icon_sim5_sprd
    };
    public static final String SIM_SLOT_NAME[] = {
    ...
    */
    ...
    public abstract Drawable getAccountIcon(AccountWithDataSet account);
    public abstract String getSimSlotName(int phoneId);
    ...
    public boolean isExistInSimSlotName(String simName) {
        for (String name : AccountTypeManager.SIM_SLOT_NAME) {
...
class AccountTypeManagerImpl extends AccountTypeManager
    ...
    */
    ...
    private List<AccountWithDataSet> mSimAccounts = Lists.newArrayList();
    /**
    * @}
    */
    ...
    protected void loadAccountsInBackground() {
         ...
         * @{
         */
        final List<AccountWithDataSet> simAccounts = Lists.newArrayList();
        /**
         * @}
         */
        try {
            ...
            for (SyncAdapterType sync : syncs) {
                ...
                } else if (USimAccountType.ACCOUNT_TYPE.equals(type)) {
                    accountType = new USimAccountType(mContext, auth.packageName);
            ...
            if (syncable) {
                ...
                if (accountTypes != null) {
                    ...
                    for (AccountType accountType : accountTypes) {
                         ...
                         * @{
                         */
                        if (accountType.accountType.matches("^sprd.*sim$")) {
                            simAccounts.add(accountWithDataSet);
                        }
                        if (accountType.accountType.matches("^sprd.*phone$")) {
         ...
         * @{
         */
        Collections.sort(simAccounts, ACCOUNT_COMPARATOR);
        /**
         * @}
         */
    ...
    */
    ...
    @Override
    public Drawable getAccountIcon(AccountWithDataSet account) {
        if (account == null) {
            return null;
        }
        ...
        if (UniverseUtils.UNIVERSEUI_SUPPORT) {
            Drawable simIcon = getSimIcon(account.type, account.name);
            return simIcon;
        }
        //
        ...
        String slot = AccountManager.get(mContext).getUserDataPrivileged(
    ...
    public boolean isSimAccount(AccountWithDataSet account) {
        ...
        for (Account a : mSimAccounts) {
            if (a.equals(account)) {
                return true;
            }
        }
        if (UniverseUtils.UNIVERSEUI_SUPPORT) {
            if (account.type.equalsIgnoreCase(ACCOUNT_SIM)
                    || account.type.equalsIgnoreCase(ACCOUNT_USIM)) {
                return true;
            }
        }
        return false;
    ...
    // add for Universe UI
    private Drawable getSimIcon(String accountType, String accountName) {
        ...
        if (accountType.equalsIgnoreCase(ACCOUNT_SIM) || accountType.equalsIgnoreCase(ACCOUNT_USIM)) {
            int simNum = TelephonyManager.getPhoneCount();
            for (int i = 0; i < simNum; i++) {
                if ((i < simIconRes.length) &&
                        accountName.equalsIgnoreCase(NAME_SIM + (i + 1))) {
                    return mContext.getResources().getDrawable(simIconRes[i]);
                }
            }
        }
