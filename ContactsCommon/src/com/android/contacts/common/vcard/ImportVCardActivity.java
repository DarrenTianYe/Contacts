...
/**
* SPRD:
*   fix bug104095 When importing some vcf files,dialog shows too fast,we can not see clearly what is written.
*
...
* @{
*/
public class ImportVCardActivity extends Activity implements Callback {
/**
* @}
*/
    ...
    */
    ...
    private class ImportRequestConnection implements ServiceConnection {
        ...
        public void onServiceConnected(ComponentName name, IBinder binder) {
            /**
            * SPRD:
            *   fix bug135973 IllegalArgumentException-Service not registered
            *
            * Original Android code:
            * 
            * 
            ...
            try {
                ...
                if (mSource != null) {
                    ...
                    for (Uri sourceUri : mSourceUris) {
                        ...
                        /**
                        * SPRD:
                        *   fix bug210012 vcf importing failed due to special file name
                        *
                        * Original Android code:
                        ...
                        */
                        ...
                        try {
                            ...
                            /**
                            * SPRD:
                            *   fix bug107274 'openable' is not supported by mms
                            *
                            * Original Android code:
                            * 
                            * 
                ...
                */
                ...
                msg.arg1=MSG_DIALOG_CACHE_DISMISS;
                if(mHandler != null){
                    mHandler.sendMessageDelayed(msg, DIALOG_DIMISS_SPAN);
                }
                /**
                * @}
                */
            ...
            try {
                ...
                try {
                    ...
                    try {
                        ...
                        /**
                        * SPRD:
                        *   210012 vcf importing failed due to special file name
                        *
                        * Original Android code:
        ...
        public void onClick(DialogInterface dialog, int which) {
            if (which == DialogInterface.BUTTON_POSITIVE) {
                if (mSelectedIndexSet != null) {
                    /**
                    * SPRD:
                    *   fix bug152868 import vcard from sd to phone, PB is finished by force
                    *
                    * Original Android code:
                    * 
                    * 
                    * @{
                    */
                    if (mSelectedIndexSet.isEmpty()) {
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                Toast.makeText(getApplicationContext(), R.string.not_select_vCard,
                    ...
                    */
                        ...
                        // We'd like to sort the files by its index, so we do
                        // not use Set iterator.
    ...
    private class VCardScanThread extends Thread implements OnCancelListener, OnClickListener {
        ...
        public void run() {
            ...
            */
            ...
            if (mGotIOException) {
                ...
                * @{
                */
                msg.arg1=MSG_DIALOG_IO_EXCEPTION;
                if(mHandler != null){
                    mHandler.sendMessageDelayed(msg, DIALOG_DIMISS_SPAN);
                }
                /**
                ...
                */
                ...
                if (size == 0) {
                     ...
                     * @{
                     */
                    msg.arg1=MSG_DIALOG_VCARD_NOT_FOUND;
                    if(mHandler != null){
                        mHandler.sendMessageDelayed(msg, DIALOG_DIMISS_SPAN);
                    }
                     /**
        ...
        private void getVCardFileRecursively(File directory)
            ...
            for (File file : directory.listFiles()) {
                /**
                * SPRD:
                *   fix bug112773 Import contacts ， can not identify  vcard file on the T card
                *
                * Original Android code:
                * 
                * 
                * @{
                */
                if(!file.equals(Environment.getExternalStoragePath())){
                /**
                * @}
                */
                    if (mCanceled) {
    ...
    private void startVCardSelectAndImport() {
        ...
        */
        ...
        if (getResources().getBoolean(R.bool.config_import_all_vcard_from_sdcard_automatically) ||
            ...
            * @{
            */
            msg.arg1 = MSG_IMPORT_FROM_SDCARD;
            if (mHandler != null) {
                mHandler.sendMessageDelayed(msg, DIALOG_DIMISS_SPAN);
            }
            /**
            ...
            * @{
            */
            msg.arg1 = MSG_DIALOG_SELECT_IMPORT_TYPE;
            if (mHandler != null) {
                mHandler.sendMessageDelayed(msg, DIALOG_DIMISS_SPAN);
            }
            /**
            ...
            * @{
            */
            msg.arg1 = MSG_DIALOG_SELECT_ONE_VCARD;
            if (mHandler != null) {
                mHandler.sendMessageDelayed(msg, DIALOG_DIMISS_SPAN);
            }
            /**
    ...
    private void importVCardFromSDCard(final List<VCardFile> selectedVCardFileList) {
        ...
        for (VCardFile vcardFile : selectedVCardFileList) {
            ...
            * @{
            */
            uriStrings[i] = "file://" + UriUtils.encodeFilePath(vcardFile.getCanonicalPath());
            /**
            * @}
            */
            i++;
    ...
    private void importVCardFromSDCard(final VCardFile vcardFile) {
        ...
        * @{
        */
        importVCard(new Uri[] {Uri.parse("file://"+UriUtils.encodeFilePath(vcardFile.getCanonicalPath()))});
        /**
        * @}
        */
    }
    ...
    protected void onCreate(Bundle bundle) {
        ...
        if (intent != null) {
            ...
            * @{
            */
            mDeviceType = intent.getIntExtra("device_type", -1);
            /**
            * @}
            */
    ...
    protected Dialog onCreateDialog(int resId, Bundle bundle) {
        switch (resId) {
            ...
            case R.id.dialog_vcard_not_found: {
                /**
                * SPRD:
                *   fix bug130250 the dialog of no vacrd in sd card is wrong for import from internal storage
                *
                * Original Android code:
                ...
                */
                String message;
                if (Constants.STORAGETYPE == 0){
                    message = getString(R.string.import_failure_no_vcard_file);
                }else{
                    if (mDeviceType == R.string.import_from_sdcard){
                        message = getString(R.string.import_failure_no_vcard_file);
                    }else{
                        message = getString(R.string.no_vcard_found_in_internal_stor);
                    }
                }
                //final String message = getString(R.string.import_failure_no_vcard_file);
                /**
    ...
    */
    ...
    super.onStop();
    if (mProgressDialogForCachingVCard!=null) {
        mProgressDialogForCachingVCard.cancel();
    }
    if (mProgressDialogForScanVCard!=null) {
        //mProgressDialogForScanVCard.cancel();
    } 
    }
    /**
    * @}
    */
    ...
    private void doScanExternalStorageAndImportVCard() {
        ...
        /**
        * SPRD:
        *   fix bug128756 the internla and external storage is wrong for 7702 nand
        *
        * Original Android code:
        ...
        */
        final File file;
        if (Constants.STORAGETYPE == 0) {
            file = Environment.getExternalStoragePath();
        } else {
            if (mDeviceType == R.string.import_from_sdcard) {
                file = Environment.getExternalStoragePath();
            } else {
                file = Environment.getInternalStoragePath();
            }
        }
        /**
        * @}
    ...
    * @{
    */
    private final static int DIALOG_DIMISS_SPAN=2000;
    private final static int MSG_DIALOG_IO_EXCEPTION=0;
    private final static int MSG_DIALOG_VCARD_NOT_FOUND=1;
    private final static int MSG_DIALOG_SELECT_IMPORT_TYPE=2;
    private final static int MSG_DIALOG_SELECT_ONE_VCARD=3;
    private final static int MSG_IMPORT_FROM_SDCARD=4;
    private final static int MSG_DIALOG_CACHE_DISMISS=5;
    ...
    public boolean handleMessage(Message msg) {
        ...
        if((mProgressDialogForScanVCard!= null) && mProgressDialogForScanVCard.isShowing()){
            mProgressDialogForScanVCard.dismiss();
            mProgressDialogForScanVCard = null;
        }
        switch(msg.arg1){
        case MSG_DIALOG_IO_EXCEPTION:
            showDialog(R.id.dialog_io_exception);
            break;
        case MSG_DIALOG_SELECT_IMPORT_TYPE:
            showDialog(R.id.dialog_select_import_type);
            break;
        case MSG_DIALOG_SELECT_ONE_VCARD:
            showDialog(R.id.dialog_select_one_vcard);
            break;
        case MSG_DIALOG_VCARD_NOT_FOUND:
            showDialog(R.id.dialog_vcard_not_found);
            break;
        case MSG_IMPORT_FROM_SDCARD:
            if(mAllVCardFileList != null){
                importVCardFromSDCard(mAllVCardFileList);
            }
            break;
        case MSG_DIALOG_CACHE_DISMISS:
            if (mProgressDialogForCachingVCard != null
                    && mProgressDialogForCachingVCard.isShowing()) {
                mProgressDialogForCachingVCard.dismiss();
                mProgressDialogForCachingVCard = null;
            }
            finish();
