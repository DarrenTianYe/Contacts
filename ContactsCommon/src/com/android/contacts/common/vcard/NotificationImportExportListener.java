...
public class NotificationImportExportListener implements VCardImportExportListener,
    ...
    public NotificationImportExportListener(Activity activity) {
        ...
        * SPRD:
        *   Bug 271822
        *    Notice: Confirm Use the same builder when refresh the progressbar.
        *
        * @{
        */
    ...
    public void onImportProcessed(ImportRequest request, int jobId, int sequence) {
        ...
        /**
        * SPRD:
        *   fix bug74484 there is too much notification when importing from sdcard
        *
    ...
    public void onImportParsed(ImportRequest request, int jobId, VCardEntry entry, int currentCount,
        ...
        /**
        * SPRD:
        *   fix bug142623 when importing vcf, empty .vcf should be reported
        *
        * Original Android code:
        ...
        */
        ...
        if (entry.isIgnorable()) {
            final Notification notification = constructImportFailureNotification(context, context.getString(R.string.fail_reason_empty));
            mNotificationManager.notify(FAILURE_NOTIFICATION_TAG, 0, notification);
            return;
        }
        /**
        * @}
    ...
    public void onImportFinished(ImportRequest request, int jobId, Uri createdUri) {
        ...
        /**
        * SPRD:
        *   Bug272958 The contact interface, menu, choose import/export - when
        *   contact import/export, notification bar in the clues shows three times
        *
        * Original Android code:
    ...
    public void onCancelRequest(CancelRequest request, int type) {
        ...
        /**
        * SPRD:
        *   fix bug201707 Finish / Cancel appears in the notification
        *
        * Original Android code:
        ...
        * @{
        */
        mNotificationManager.notify(FAILURE_NOTIFICATION_TAG, request.jobId, notification);
        /**
        * @}
        */
    }
    ...
    /* package */ static Notification constructProgressNotification(
        ...
        */
        mBuilder.setOngoing(true)
                ...
                .setContentIntent(PendingIntent.getActivity(context, 0, intent, 0));
        if (totalCount > 0) {
            mBuilder.setContentText(context.getString(R.string.percentage,
                    String.valueOf(currentCount * 100 / totalCount)));
        }
    ...
    */
    ...
    @Override
    public void onImportException(int jobId, Exception e) {
    final String description = mContext.getString(R.string.reading_vcard_exception_title);
    final Notification notification =
        NotificationImportExportListener.constructExceptionNotification(mContext, description,e);
    mNotificationManager.notify(NotificationImportExportListener.FAILURE_NOTIFICATION_TAG,
                    0, notification);
    }

    static Notification constructExceptionNotification(Context context, String description, Exception e) {
        String text=description;
        if (e instanceof SQLiteFullException
            || e instanceof SQLiteDiskIOException) {
            text=context.getString(R.string.sqlite_full);
        }
        return mBuilder
            .setAutoCancel(true)
            .setTicker(text)
